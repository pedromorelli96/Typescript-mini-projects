...
import { todoSchema, type Todo } from "shared";
import { z } from "zod";
...
type DeleteManyArgs = { ids: Array<Todo["id"]> } | { clearAll: true };
...
export class Db {
  ...
  private async readJson(): Promise<ReadJsonResult> {
    try {
      ...
      const deserializedTodos: unknown = JSON.parse(data);
      const schema = z.array(todoSchema);
      const parsedData = schema.parse(deserializedTodos);
      ...
      if (notFoundError.success) {
        ...
        console.error(`Invalid JSON at ${this.filePath}`);
        return { success: false, error: new Error(`Invalid JSON at ${this.filePath}`) };
      } else if (error instanceof z.ZodError) {
        console.error(`Invalid data at ${this.filePath}`);
  ...
  public async create(args: CreateArgs): Promise<Todo> {
    ...
    if (!readResult.success) throw new Error("Error reading todos!");
  ...
  public async findUnique(args: FindUniqueArgs): Promise<Todo | null> {
    const readResult = await this.readJson();
    if (!readResult.success) throw new Error("Error reading todos!");
    return readResult.todos.find((todo) => todo.id === args.id) ?? null;
  }
  ...
  public async update(args: UpdateArgs): Promise<Todo | null> {
    ...
    const updatedTodo = {
      ...readResult.todos[todoIndex],
      ...args.data,
    } satisfies Todo;
    readResult.todos[todoIndex] = updatedTodo;
    await this.writeJson(readResult.todos);
    return updatedTodo;
  }
